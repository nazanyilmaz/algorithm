...
function sherlockAndMinimax(arr, p, q) {
  // Sort the array to facilitate searching for minimum distances
  arr.sort((a, b) => a - b);

  // Initialize variables to track the best M and the maximum minimum distance
  let bestM = p;
  let maxMinDistance = -1;

  // Check boundaries p and q
  const candidates = [p, q];

  // Add elements from arr that are within the range
  ...
  for (const M of candidates) {
    // Calculate the minimum distance to arr
    let minDistance = Infinity;

    for (const value of arr) {
      const distance = Math.abs(value - M);
      if (distance < minDistance) {
        minDistance = distance;
      }
    }

    // Check if this minDistance is the largest we've seen
    if (minDistance > maxMinDistance) {
      maxMinDistance = minDistance;
      bestM = M;
    } else if (minDistance === maxMinDistance) {
      // If we found a tie, choose the smaller M
