...
function luckBalance(k, contests) {
  ...
  for (const contest of contests) {
    ...
    if (importance === 1) {
      ...
      totalLuck += luck; // Add luck of unimportant contests directly
    }
  }
  ...
  // Calculate the luck balance from important contests
  // Lose k important contests (highest luck values)
  for (let i = 0; i < importantContests.length; i++) {
    if (i < k) {
      totalLuck += importantContests[i]; // Lose these contests
    } else {
      totalLuck -= importantContests[i]; // Win these contests
    }
  }
